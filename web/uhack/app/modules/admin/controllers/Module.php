...
class Module extends MX_Controller 
{
	...
	private function _modules_add()
	{
		...
		$this->template->add_css('mods/fontawesome-iconpicker/css/fontawesome-iconpicker.min.css');	
		$this->template->add_js('mods/fontawesome-iconpicker/js/fontawesome-iconpicker.min.js');		
		// $this->template->add_js('mods/input_mask/input_mask.min.js');
	...
	private function _add_fields($string, $module)
	{
		...
		foreach ($column_names as $column_name)
		{
			...
			if ($column_types[$key] == 'DATETIME')
			{
				...
				$embed_datetime_plugin = "\$this->template->add_js('mods/datetimepicker/DateTimePicker.min.js'); \n\t\t";
				$embed_datetime_plugin .= "\$this->template->add_css('mods/datetimepicker/DateTimePicker.css');";
			}

			if ($column_types[$key] == 'DATE')
			{
				 $view_form_field_date = "\$('.datepicker').datepicker({\n\t\tchangeMonth: true,\n\t\tchangeYear: true,\n\t\tdateFormat: 'yy-mm-dd'\n\t});";
				 $embed_jqueryui_plugin = "\$this->template->add_js('mods/jquery-ui/ui/widgets/datepicker.js'); \n\t\t";
				 $embed_jqueryui_plugin .= "\$this->template->add_css('mods/jquery-ui/themes/base/all.css'); \n\t\t";
			}
